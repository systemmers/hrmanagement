...
class TestCorporateSettingsAPI:
    ...
    def test_get_settings_success(self, auth_client_corporate_full):
        """설정 조회 성공"""
        with patch('app.domains.company.blueprints.settings.settings_api.corporate_settings_service') as mock_service, \
             patch('app.domains.company.blueprints.settings.settings_api.get_company_id', return_value=1):
    ...
    def test_get_settings_no_company_id(self, auth_client_corporate_full):
        """법인 ID 없을 때"""
        with patch('app.domains.company.blueprints.settings.settings_api.get_company_id', return_value=None):
            response = auth_client_corporate_full.get('/api/corporate/settings')
