...
시스템 설정 서비스 테스트
Phase 30 이후: PropertyMock을 사용하여 property mock
"""
import pytest
...
def mock_repo():
    ...
    mock = Mock()
    with patch.object(SystemSettingService, 'system_setting_repo', new_callable=PropertyMock, return_value=mock):
        yield mock
...
class TestGetByKey:
    ...
    def test_get_by_key_success(self, mock_repo):
        """설정 조회 성공"""
    ...
    def test_get_by_key_not_found(self, mock_repo):
        """설정을 찾을 수 없을 때"""
...
class TestGetValue:
    ...
    def test_get_value_dict_success(self, mock_repo):
        """Dict 형태 설정 값 조회"""
        mock_setting = {'key': 'test.key', 'value': 'test_value'}
        mock_repo.get_by_key.return_value = mock_setting
    ...
    def test_get_value_object_success(self, mock_repo):
        """객체 형태 설정 값 조회"""
        mock_setting = Mock()
    ...
    def test_get_value_with_default(self, mock_repo):
        """기본값과 함께 조회"""
        mock_repo.get_by_key.return_value = None
    ...
    def test_get_value_no_value_key(self, mock_repo):
        """value 키가 없는 Dict"""
...
class TestGetCompanyData:
    ...
    def test_get_company_data_success(self, mock_repo):
        """회사 정보 조회 성공"""
    ...
    def test_get_company_data_partial(self, mock_repo):
        """일부 회사 정보만 있을 때"""
        def mock_get_by_key(key):
            """Mock 설정 반환 - 일부만"""
            if key == 'company.name':
    ...
    def test_get_company_data_object_type(self, mock_repo):
        """객체 형태 설정 값"""
        def mock_get_by_key(key):
            """Mock 객체 반환"""
            if key == 'company.name':
    ...
    def test_get_company_data_empty(self, mock_repo):
        """회사 정보가 없을 때"""
        mock_repo.get_by_key.return_value = None
...
class TestSystemSettingServiceIntegration:
    ...
    def test_get_value_various_defaults(self, mock_repo):
        """다양한 기본값 타입"""
        mock_repo.get_by_key.return_value = None
